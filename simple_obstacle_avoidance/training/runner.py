"""
Custom runner for Simple Obstacle Avoidance Task.

This script registers our custom task and environment with aerial_gym and rl_games,
then runs PPO training.

Usage:
    cd /workspaces/aerial_gym_docker
    python -m simple_obstacle_avoidance.training.runner --file=simple_obstacle_avoidance/training/ppo_simple_obstacle.yaml --train
"""
import isaacgym
import argparse
import os
...
from simple_obstacle_avoidance.config.robot_config import CustomQuadWithCameraCfg
...
# Register our sparse obstacle environment configuration
env_config_registry.register("simple_obstacle_env", SimpleObstacleEnvCfg)

# Register our custom robot with modified spawn position
robot_registry.register("custom_quadrotor_with_camera", BaseMultirotor, CustomQuadWithCameraCfg)

# Register our task with the task registry
...
class ExtractObsWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        # Combine termination and truncation into single done signal
        dones = torch.where(
...
# Register our task with rl_games env_configurations
env_configurations.register(
    "simple_obstacle_avoidance_task",
    {
        "env_creator": lambda **kwargs: task_registry.make_task(
            "simple_obstacle_avoidance_task", **kwargs
        ),
...
def get_args():
    custom_parameters = [
        ...
        {"name": "--use_warp", "type": lambda x: bool(strtobool(x)), "default": "True", "help": "Use warp"},
        {"name": "--experiment_name", "type": str, "default": "simple_obstacle_avoidance", "help": "Experiment name"},
        {"name": "--task", "type": str, "default": "simple_obstacle_avoidance_task", "help": "Task name"},
    ...
    args = parse_arguments(description="Simple Obstacle Avoidance", custom_parameters=custom_parameters)
...
def update_config(config, args):
    ...
    # Update num_actors to match num_envs
    if args["num_envs"] > 0:
